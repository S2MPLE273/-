...
- 当前阶段：v0.2.3 回收站清理修复（提权直删）+ 失败项原始错误显示，已人工验收（70/70 测试）；v0.2.2 密钥格式 v2（精确 24h）+ keygen 密码门禁/历史记录；v0.2.1 修复切盘按盘过滤/TOP10 超时/进度条确定式 0→100；v0.2 新增进度条+动效、已选统计、管理员接口（X-Admin-Key 三端点 + 服务方管理面板）
...
3. 密钥位布局 v2（`[[2,2],[issueDay,16],[validDays,6],[tagHash,16],[issueHour,5],[issueMinute,6],[0,1]]` + HMAC 截 3 字节，version=2，v1 已作废）是 license.js 与 keygen 模板的硬约束，改动必须跑交叉测试
