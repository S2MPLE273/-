...
>
> **v0.2.5（2026-08-17，独立代码审查后修正）**：
> - 生效容差收紧为 **4h**（24h 容差经审查量化：恶意客户首次拨慢时钟可把 1 天密钥用成约 2 天；服务方拍板 4h）。超出容差返回 `not_yet_valid`（"密钥尚未生效"），与 `clock_rollback`（保留给 lastSeen 回拨检查）拆分为两种提示。
> - **lastSeen 回拨容差 1h**：`cur < rec.lastSeen - 1h` 才判回拨，吸收"时钟先快后被 NTP 校时回调"的假拒绝。
> - generate 对归一化后非 32 hex 的 deviceCode 抛错（防静默生成永不匹配的绑定密钥）。
...
3. 有效期：`当前时刻 ∈ [签发时刻(分钟取整) - 4h 容差, 签发时刻 + validDays×24h]`（v0.2.5 起，见 §4.1 附注）→ 过期提示「密钥已过期，请联系服务人员获取新密钥」
...
6. 时间回拨防护：状态文件记录 `lastSeen`（最后成功验证的时间）；若当前时间早于 `lastSeen - 1h` → 拒绝（防改系统时间绕过有效期；1h 容差吸收 NTP 校时回调，v0.2.5 起）
